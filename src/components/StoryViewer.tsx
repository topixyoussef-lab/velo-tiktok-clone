  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex items-center justify-center">
      ...
      >
        ...
        {showComments && (
          <div className="absolute inset-0 bg-black/80 flex flex-col z-10" onClick={() => { setShowComments(false); pausedRef.current = false; startTimer(); }}>
            <div className="flex items-center justify-between p-4 border-b border-white/10" onClick={e => e.stopPropagation()}>
              <h3 className="text-white font-semibold">Comments ({commentsCount})</h3>
              <button onClick={() => { setShowComments(false); pausedRef.current = false; startTimer(); }} className="text-white/60 hover:text-white">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col gap-4">
                  {comments.map(c => (
                    <div key={c.id} className="flex gap-3">
                      <img src={c.avatar || '/default-avatar.svg'} className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
                      <div>
                        <p className="text-white text-sm"><span className="font-semibold">{c.username}</span> {c.text}</p>
                        <p className="text-white/30 text-[10px] mt-0.5">{new Date(c.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
