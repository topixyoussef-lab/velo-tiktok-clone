      ...
      const interval = setInterval(() => {
        try {
          if (type === 'messages' && otherId) {
            ...
            `).all(userId, otherId, otherId, userId, lastMsgTime || '1970-01-01');
            ...
            const count = db.prepare('SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND read = 0').get(userId) as { c: number };
            ...
            `).all(userId, lastNotifTime || '1970-01-01');

            if (newNotifs.length > 0) {
              lastNotifTime = (newNotifs[0] as any).created_at;
              const unread = db.prepare('SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND read = 0').get(userId) as { c: number };
